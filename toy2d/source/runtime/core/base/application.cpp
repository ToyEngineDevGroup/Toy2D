...
namespace Toy2D {
    ...
    Application::Application(const std::filesystem::path& game_root_path) {
        ...
        m_audio_manager    = CreateScope<AudioManager>(m_window->getNativeWindowHandle());
        ...
        while (m_running) {
            ...
            if (elapsed_time > 1.0f) {
                std::wstring window_text =
                    L"Toy2D Engine - " + std::to_wstring((int)ImGui::GetIO().Framerate) + L" fps";
