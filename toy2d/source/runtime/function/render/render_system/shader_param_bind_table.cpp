...
namespace Toy2D {
    ...
    void ShaderParamBindTable::registerShader(
        ...
        switch (usage) {
            case ShaderUsage::BASIC: {
                auto shader                              = Toy2D::CreateRef<BasicShader>(properties, RenderContext::getInstance().getGraphicsDevice(), samplers);
    ...
    void ShaderParamBindTable::registerShader(
        ...
        switch (usage) {
            case ShaderUsage::BASIC: {
                auto shader                              = Toy2D::CreateRef<BasicShader>(properties, std::move(rootSig));
                m_shader_bind_table[shader.get()]        = TParamBindTable();
                m_shader_table[std::string(shader_name)] = std::move(shader);
                break;
            }
